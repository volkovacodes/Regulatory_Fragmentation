...
outfolder <- "M:/evolkova/Dropbox/Projects/Govt Agenda/Data/LDA_Data/LDA10K/"
...
for(yr in 1994:2019)
{
  ...
  docsind <- which(!rownames(dtm) %in% lda$filename)
  
  print(yr)
  print(Sys.time())
  print(length(docsind))
  
  res <- mclapply.hack(docsind,ldafit)
  
  #lda <- NULL
  for(i in 1:length(res)) 
  {
    tmp <- data.frame(res[[i]], filename = rownames(res[[i]]))
    colnames(tmp)[1:100] <- paste0("Topic", 1:100)
    rownames(tmp) <- NULL
