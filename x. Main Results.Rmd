...
mainx <- c(rep("regul.disp",2), rep("topic.disp + regul.disp",3))
#### fix here !!!
controls <- controls_min
fe <- c(rep(c("|FF48 + year", "|FF48 + year + cik"),2), "|FF48 + cik + FF48_year")
...
**Table 10 Regulatory Dispersion across Notices, Proposed Rules, and Final Rules**
...
reg_type <- function(tp) {
  yvars <- c("lead.sga_at", "lead.tfp", "lead.growth", "lead.growth.at", "lead.roa","lead.emp_at")
...
**Table 12. Aggregation topics in groups**
...
group.regs <- function(yvar){
  ...
  line2 <- paste0(yvar, " ~ regul.disp.gr + regul.complex.log.gr + topic.disp + LogWords10K + ppe_at + ebitda_at + log_sale + tobin|FF48 + FF48_year + cik")
