...
p <- ggplot(data = data, aes(disp)) +
  ...
  xlab("Dispersion of agencies per topic") + ylab("Density [% of topic-year obs]") #+ 
...
p <- ggplot(data = companyyear, aes(topic.disp)) +
  ...
  theme_minimal() + xlab("Topic Dispersion in Firms") + 
...
p <- ggplot(data = companyyear[abs(regul.disp) < 1.1], aes(regul.disp)) +
  ...
  theme_minimal() + 
  xlab("Regulation Dispersion") + ylab("Density [% of firm-year obs]") 
print(p)
```
...
difgraph <- function(int1, int2, lab1, lab2, data) {
  ...
  plot <- increases %>%
    ...
    scale_fill_brewer(palette = "Paired") + ylab("Agency Dispersion") 
